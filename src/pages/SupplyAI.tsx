...
function getRecommendation(outlet: OutletData): Recommendation {
  ...
  if (trend === "growing" && sell_through >= 85) {
    ...
    return {
      ...
      color: "#10B981",
      bgColor: "#ECFDF5",
      suggested: delivered + increase,
      change: increase,
      impact: `+N$${(increase * 4.5).toFixed(0)}`,
      reason: `${sell_through.toFixed(0)}% sell-through & growing — avoid lost sales`,
      confidence: 92,
    };
  }
  if (trend === "declining" || return_rate > 35) {
    ...
    return {
      ...
      color: "#EF4444",
      bgColor: "#FEF2F2",
      suggested: delivered - reduction,
      change: -reduction,
      impact: `Save N$${(reduction * 2.8).toFixed(0)}`,
      reason: `${return_rate.toFixed(0)}% return rate — cut waste & costs`,
      confidence: 88,
    };
  }
  if (sell_through >= 75) {
    ...
    return {
      ...
      change: increase,
      impact: `+N$${(increase * 4.5).toFixed(0)}`,
      reason: `Healthy ${sell_through.toFixed(0)}% sell-through — small uplift recommended`,
      confidence: 74,
    };
  }
  return {
    ...
    change: 0,
    impact: "Optimised",
    reason: `Stable at ${sell_through.toFixed(0)}% sell-through`,
    confidence: 81,
  };
}
    ...
    >
      ...
      >
        ...
        <div style={{ width: 100, flexShrink: 0 }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 600, color: barColor, marginTop: 2, textAlign: "right" }}>
            {outlet.sell_through.toFixed(1)}%
          </div>
        </div>
        ...
        <div style={{ padding: "0 16px 16px", animation: "fadeUp 0.25s ease-out" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 12 }}>
            ...
            <div style={{ background: rec.bgColor, borderRadius: 8, padding: "12px 14px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 600, color: rec.color }}>
                {rec.change > 0 ? `+${rec.change}` : rec.change} copies
              </div>
            </div>
            <div style={{ background: "#F3F4F6", borderRadius: 8, padding: "12px 14px", textAlign: "center" }}>
          ...
          <div style={{
            ...
            {rec.action === "reduce" || rec.action === "maintain" ? (
              <AlertCircle size={16} color={rec.color} />
            ) : (
            ...
            <span style={{
              fontSize: 11,
              fontWeight: 600,
              color: rec.confidence >= 85 ? "#10B981" : "#F59E0B",
            }}>
...
// --- Main Page ---
export default function SupplyAI() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (monthlyRaw && monthlyRaw.length > 0) {
        setMonthlySummary(
          monthlyRaw.map((r) => ({
            ...
            returned: Number(r.total_returns),
          }))
        );
      } else {
      ...
      if (cleanRaw && cleanRaw.length > 0) {
        // Group by shop, take latest month, build sparkline from last 6 months
        const shopMap = new Map<string, typeof cleanRaw>();
        cleanRaw.forEach((r) => {
          const name = r.shop_name || r.shop_id;
          if (!shopMap.has(name)) shopMap.set(name, []);
          shopMap.get(name)!.push(r);
        });
        ...
        shopMap.forEach((rows, name) => {
          ...
          const sparkline = sorted.slice(-6).map((r) => {
            const d = (Number(r.quantity_sold) || 0) + (Number(r.quantity_returned) || 0);
            return d > 0 ? ((Number(r.quantity_sold) || 0) / d) * 100 : 0;
          });
          ...
          outletList.push({
            ...
            trend: deriveTrend(returnRate, sellThrough),
        ...
        setOutlets(
          outletFallback.map((o) => ({
            ...o,
            sell_through: o.delivered > 0 ? (o.sold / o.delivered) * 100 : 0,
            sparkline: Array.from({ length: 6 }, () => 60 + Math.random() * 35),
          }))
        );
      }
    } catch {
  ...
  // Derived stats from latest month
  ...
  const increaseCount = useMemo(() => outlets.filter((o) => {
    const rec = getRecommendation(o);
    return rec.action === "increase" || rec.action === "slight_increase";
  }).length, [outlets]);
          ...
          <div style={{ background: "#0D1117", padding: "24px 28px 0" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#10B981", textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 2 }}>
                    AI Supply Intelligence
                  </div>
                  <h1 style={{ fontSize: 22, fontWeight: 700, color: "#F9FAFB", margin: 0, fontFamily: "'DM Sans', sans-serif" }}>
                    Demand & Distribution Advisor
                  </h1>
                </div>
              </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="fade-up-d1" style={{
                  ...
                  <h2 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: "0 0 16px", fontFamily: "'DM Sans', sans-serif" }}>
                      ...
                      {filterButtons.map((btn) => (
                        <button
                          ...
                          style={{
                            padding: "6px 14px",
                            borderRadius: 20,
                            border: filter === btn.key ? "2px solid #10B981" : "1px solid #E5E7EB",
                            background: filter === btn.key ? "#ECFDF5" : "#fff",
                            color: filter === btn.key ? "#065F46" : "#6B7280",
                            fontSize: 12,
                            fontWeight: 600,
                            cursor: "pointer",
                            fontFamily: "'DM Sans', sans-serif",
